...
function Like({ id, uid, number, timestamp, seen, type }) {
    ...
    const changeNotifyState = () => {
        db.collection('users').doc(`${user.uid}`).collection('notification').doc(`${id}-likes`).update({
            seen: true,
        })
    }
            ...
            <a>
                <div onClick={ changeNotifyState } className="flex space-x-5 cursor-pointer relative hover:bg-gray-200 p-2 rounded-xl">
                    <div className="mt-1 relative">
                        ...
                        <div className="absolute -right-3 top-8 rounded-full p-1 bg-blue-400 flex items-center">
                            <ThumbUpIcon fontSize="small" className="text-white"/>
                        </div>
                    </div>
                    <div className="space-x-1 text-gray-700">
                        ...
                        { number > 1 && 
                            <>
                                <span>và</span>
                                <span className="font-semibold">{`${number - 1} người khác`}</span>
                            </>
                        }
                        <span>đã thích {type ? 'ảnh' : 'bài viết'} của bạn</span>
